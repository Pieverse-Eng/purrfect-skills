---
name: bybit
description: Bybit Pay merchant integration — accept crypto payments via QR Payment (one-time scan-to-pay) and Recurring Payments (subscription / on-demand / single-use auto-deduction agreements). Covers signing, webhook verification, idempotency, polling fallbacks, and risk fields.
---
...
> See full field reference: [Create Payment API](scan-payment/create-payment.mdx) · [Refund API](scan-payment/refund.mdx)
...
7. POST /v5/bybitpay/agreement/refund  → refund if needed (see [Refund API](recurring-payments/refund.mdx))
8. POST /v5/bybitpay/agreement/unsign  → terminate when user cancels (see [Unsign API](recurring-payments/unsign.mdx))
```
...
> See full field reference: [Sign Request API](recurring-payments/sign-request.mdx)
...
> See full field reference: [Deduction API](recurring-payments/deduction.mdx)
...
See full structure: [Payment Notify](scan-payment/payment-notify.mdx)
...
See full structure: [Webhooks Overview](recurring-payments/webhooks/overview.mdx)
