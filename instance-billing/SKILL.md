...
description: Use when a hosted Purrfect Claw user asks about their plan, expiration, AI credits, payment methods, credit recharge/top-up, subscription renewal, extension, billing status, 充值, 续期, or paying with a named token such as PIEVERSE, USDC, USDT, ETH, BNB, or $U.
---
...
4. A direct instruction containing a valid quantity, such as “充 100 credits”, is authorization for this one payment. Use `--yes` without asking again.
...
“续一下期” means one 30-day cycle. A direct renewal instruction is authorization for that one payment.
...
- `confirming`: state that payment was broadcast and fulfillment is still pending; include the invoice and transaction identifiers, then use `purr instance billing-status --invoice <invoice-id>` for a later read-only check.
