...
Use the Glob tool to find `vendor/common/pool-apr.mjs` and note its absolute path. Then run:

```bash
PCS_POOL_APR_SCRIPT=/absolute/path/to/vendor/common/pool-apr.mjs
node "$PCS_POOL_APR_SCRIPT"
```
...
Use the Glob tool to find `vendor/common/protocol-fee.mjs` and note its absolute path. Then run:

```bash
PCS_PROTOCOL_FEE_SCRIPT=/absolute/path/to/vendor/common/protocol-fee.mjs
CHAIN_ID="$CHAIN_ID" RPC="$RPC" POOL_ID="$POOL_ID" \
  node "$PCS_PROTOCOL_FEE_SCRIPT"
```
...
Use the Glob tool to find `vendor/farming-planner/references/fetch-solana.cjs` within this skill's own directory and note its absolute path. Then set:
