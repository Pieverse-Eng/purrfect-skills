---
name: gate-exchange-unified-runtime-rules
version: "2026.4.3-1"
updated: "2026-04-03"
description: "Packaged runtime rules for gate-exchange-unified so the published skill bundle contains every mandatory guardrail referenced by SKILL.md."
---

# Gate Unified Account Runtime Rules

> Packaged runtime rules for `gate-exchange-unified`.
...
- This skill only covers unified-account operations. Route spot, futures, or unrelated account tasks to the matching skill instead of forcing them through this one.

## 3. Mandatory Confirmation Gate

- Before any write action, present an operation draft first.
- The draft must include the requested mode change or borrow/repay action, affected asset or account scope, estimated impact when available, and a risk note.
...
- On missing `gate-cli` configuration, auth failure, invalid leverage or borrow constraints, or unsupported account state, stay in read-only explanation mode.
- Explain why execution is blocked and what must be fixed.
- Surface technical failures that affect account safety.
