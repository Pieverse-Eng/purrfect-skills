...
- **User assets**: Call `gate-cli cex tradfi account assets` with only MCP-documented parameters. Extract: asset (or as returned by API), available, locked/total as returned.
- **MT5 account**: Call `gate-cli cex tradfi account info` with only MCP-documented parameters. Extract: account id, server, login, balance, equity, margin, or fields as returned.
...
1. Call `gate-cli cex tradfi account assets` with only MCP-documented parameters. If the response includes multiple assets, show the requested one or all as returned.
...
1. Call `gate-cli cex tradfi account info`.
