...
- `__ROUTE_CLI__` → stop here. Do **not** run Trigger update or the legacy MCP
  ...
  `https://github.com/gate/gate-skills/blob/master/skills/gate-info-risk/SKILL.md`
  and its playbook contract. Carry over the minimum context: `token`+`chain`,
  `address`+`chain`, or `symbol` depending on whether the query is token risk,
  address risk, or project-level risk.
