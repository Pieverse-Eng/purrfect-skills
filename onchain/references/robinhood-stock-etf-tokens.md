...
Use this reference to resolve the listed Robinhood Chain stock/ETF tickers
for balances, transfers, or swaps. For quoting, buying, selling, or swapping,
read [Robinhood Chain Token Swaps](robinhood-swaps.md).

These addresses are for Robinhood Chain only (chain ID `4663`).

## Usage Notes

- Use the listed addresses when referring to these stock/ETF products;
  a different contract with the same ticker is not the same asset.
- The `purr` CLI resolves these tickers on `--chain-id 4663` /
  `--chain robinhood` for balance, transfer, and `wallet uniswap` commands.
- This is a stock/ETF address directory, not a swap allowlist. Other Robinhood
  Chain tokens, including memecoins, can be quoted by their exact contract
  addresses through the same [swap workflow](robinhood-swaps.md).
