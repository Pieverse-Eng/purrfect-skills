...
| `lighter` | Lighter (lighter.xyz) market data, perp/spot trading, leverage and margin, multi-chain USDC open-account/deposit, secure and fast withdrawals via `purr lighter`. |
...
| Skill | Description |
|-------|-------------|
| `agentkey` | AgentKey live web search, social, crypto, and API data through hosted MCP tools. |
...
| `pieverse-staking` | Pieverse testnet BURR staking — list positions, stake, withdraw one matured stake, or batch withdraw on Sepolia / BSC Testnet. |
