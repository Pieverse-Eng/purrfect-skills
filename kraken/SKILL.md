...
Public market data does not require trading credentials. Use exact returned
instrument identifiers for subsequent queries and orders.

- Spot pairs: `kraken pairs --pair <PAIR> -o json`
- Spot price: `kraken ticker <PAIR> -o json`
- Spot candles: `kraken ohlc <PAIR> --interval <MINUTES> --since <EPOCH_SECONDS> -o json`
- For xStocks, add `--asset-class tokenized_asset` to pair, ticker, OHLC, and order-book queries.
- Futures instruments: `kraken futures instruments -o json`
- Futures ticker: `kraken futures ticker <SYMBOL> -o json`
- Futures order book: `kraken futures orderbook <SYMBOL> -o json`
