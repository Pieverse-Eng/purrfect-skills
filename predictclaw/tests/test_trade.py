...
class FakeWalletSdk:
    def __init__(
        ...
        *,
        ...
        usdt_allowance_wei: int = 25_000_000_000_000_000_000,
    ) -> None:
        self.mode = mode
        self.signer_address = signer_address
        self.funding_address = funding_address
        self._usdt_balance_wei = usdt_balance_wei
        self._usdt_allowance_wei = usdt_allowance_wei
        self._builder = FakeBuilder()
        self.approved_amounts: list[tuple[int, bool, bool]] = []
    ...
    def get_usdt_allowance_wei(
        ...
        *,
        ...
        return self._usdt_allowance_wei
    ...
    def approve_usdt(
        ...
        *,
        ...
        self.approved_amounts.append((amount_wei, is_neg_risk, is_yield_bearing))
        return {"success": True}
...
@pytest.mark.asyncio
async def test_trade_buy_predict_account_fails_closed_until_kernel_execute() -> None:
    config = PredictConfig.from_env(
        {
            "PREDICT_ENV": "testnet",
            "PREDICT_STORAGE_DIR": os.environ["PREDICT_STORAGE_DIR"],
...
@pytest.mark.asyncio
async def test_buy_approves_bounded_amount_when_allowance_insufficient() -> None:
    config = PredictConfig.from_env(
        {
            "PREDICT_ENV": "testnet",
            "PREDICT_STORAGE_DIR": os.environ["PREDICT_STORAGE_DIR"],
    ...
    api_client = FakeApiClient()
    wallet_sdk = FakeWalletSdk(usdt_allowance_wei=1_000_000_000_000_000_000)
    service = TradeService(
        config,
        api_client_factory=lambda _config, _jwt_provider: api_client,
        wallet_sdk_factory=lambda _config: wallet_sdk,
    ...
    assert wallet_sdk.approved_amounts == [
        (25_000_000_000_000_000_000, False, False)
    ]
    assert len(api_client.created_orders) == 1
    assert result.status == "FILLED"
...
@pytest.mark.asyncio
async def test_buy_skips_approval_when_allowance_is_sufficient() -> None:
    config = PredictConfig.from_env(
        {
            "PREDICT_ENV": "testnet",
            "PREDICT_STORAGE_DIR": os.environ["PREDICT_STORAGE_DIR"],
    ...
    api_client = FakeApiClient()
    wallet_sdk = FakeWalletSdk(usdt_allowance_wei=25_000_000_000_000_000_000)
    service = TradeService(
        config,
        api_client_factory=lambda _config, _jwt_provider: api_client,
        wallet_sdk_factory=lambda _config: wallet_sdk,
    ...
    await service.buy("123", "YES", "25", slippage_bps=50)

    assert wallet_sdk.approved_amounts == []
    assert len(api_client.created_orders) == 1


class NoApprovalWalletSdk:
    def __init__(self) -> None:
        self._builder = FakeBuilder()


@pytest.mark.asyncio
async def test_buy_blocks_when_bounded_approval_is_unavailable() -> None:
    config = PredictConfig.from_env(
        {
            "PREDICT_ENV": "testnet",
            "PREDICT_STORAGE_DIR": os.environ["PREDICT_STORAGE_DIR"],
    ...
    service = TradeService(
        config,
        api_client_factory=lambda _config, _jwt_provider: api_client,
        wallet_sdk_factory=lambda _config: NoApprovalWalletSdk(),
    )

    with pytest.raises(ConfigError, match="bounded USDT approval is unavailable"):
        await service.buy("123", "YES", "25")
