...
from lib.mandated_sdk_bridge import MandatedVaultBridgeError
...
def build_parser() -> argparse.ArgumentParser:
    ...
    bootstrap = subparsers.add_parser(
        ...
        description=(
            ...
            "transaction summary. Explicit confirmation is required before broadcast. With --confirm it plans through the bundled @erc-mandated/sdk helper and broadcasts the resulting transaction request through the platform wallet API."
        ),
    )
...
def _handle_status(args: argparse.Namespace) -> int:
    ...
    except (ConfigError, MandatedVaultBridgeError) as error:
    ...
    if payload.get("mode") == "mandated-vault":
        ...
        print(f"SDK Helper Ready: {'yes' if sdk.get('runtimeReady') else 'no'}")
        ...
        missing = sdk.get("missingRequiredTools", [])
        print(
            f"Missing Required SDK Tools: {', '.join(missing) if missing else 'none'}"
        )
    ...
    if payload.get("fundingRoute") == "vault-to-predict-account":
        ...
        print(f"Predict Account Order Address (execution unavailable): {payload.get('tradingIdentityAddress')}")
...
def _handle_continue_follow_up(args: argparse.Namespace) -> int:
    ...
    except (ConfigError, MandatedVaultBridgeError, json.JSONDecodeError) as error:
        return _emit_error(args, error)
...
def _handle_bootstrap_vault(args: argparse.Namespace) -> int:
    ...
    if args.confirm:
        print(f"Submitted Vault (pending receipt): {payload['deployedVault']}")
