...
def print_help() -> None:
    ...
    print("  WALLET_API_URL              Platform wallet application API base URL")
    print("  WALLET_API_TOKEN            Platform wallet bearer token (never printed)")
    print("  INSTANCE_ID                 Platform wallet instance id")
    print("  WALLET_CHAIN_ID             Optional chain id override for the platform wallet")
    ...
    print("  ERC_MANDATED_CONTRACT_VERSION Contract version passed to the mandated SDK helper")
    print("  ERC_MANDATED_CHAIN_ID       Optional explicit chain selection for the mandated SDK helper")
    print(
    ...
    print(
        "  - `wallet bootstrap-vault --confirm` plans with the bundled SDK helper and broadcasts through the platform wallet API."
    )
    print(
        "  - wallet status/deposit JSON expose manualTopUpAddress, fundingAddress, tradingIdentityAddress (compat order/recipient field; execution unavailable), orchestrationVaultAddress, vaultAuthority, vaultExecutor, bootstrapSigner, allowedTokenAddresses, and allowedRecipients when vault permissions are configured."
    )
    print(
        "  - wallet redeem-vault is preview-only and reports redeemableNow/blockingReason before any future redeem execution path."
    )
    print(
        "  - ERC_MANDATED_VAULT_ADDRESS selects an explicit deployed vault; otherwise the full derivation tuple lets the bundled SDK helper predict the vault address."
    )
    print(
        "  - Product default factory is 0x6eFC613Ece5D95e4a7b69B4EddD332CeeCbb61c6. Confirmed bootstrap returns a manual env block (envBlock/backfillEnv); it does not auto-edit .env."
    )
    print(
        "  - Overlay wallet status/deposit treat the Vault deposit flow as the default funding ingress while Predict Account stays the deposit/recipient identity; Predict Account buy/withdraw execution is currently unavailable. Undeployed vault setup remains manual-only."
    )
    print(
        "  - The internal funding route identifier for this pattern remains `vault-to-predict-account`."
    )
    print(
        "  - Trust boundary: the bundled SDK helper orchestrates transport/preparation; the vault contract policy authorizes what execution is allowed."
    )
    print(
    ...
    print(
        "  - Predict Account buy and wallet withdraw fail closed with unsupported-predict-account-execution until controlled Kernel execute lands; EOA buy/withdraw remain available."
    )
