...
from .config import (
    ...
    predict_account_execution_unsupported_error,
)
...
from .mandated_sdk_bridge import MandatedVaultBridge
...
class TradeService:
    ...
    async def buy(
        ...
        *,
        ...
        if self._config.wallet_mode == WalletMode.PREDICT_ACCOUNT:
            raise predict_account_execution_unsupported_error("buy")
        ...
        await self._ensure_bounded_usdt_allowance(
            sdk=sdk,
    ...
    async def _ensure_bounded_usdt_allowance(
        self,
        *,
        sdk: Any,
        amount_wei: int,
        ...
        getter = getattr(sdk, "get_usdt_allowance_wei", None)
        approver = getattr(sdk, "approve_usdt", None)
        if not callable(getter) or not callable(approver):
            raise ConfigError(
                "buy: bounded USDT approval is unavailable for this wallet route. "
                "Use the default platform-wallet EOA path; raw-key fallbacks cannot "
                "place buys safely."
            )
