...
from .mandated_sdk_bridge import (
...
from .platform_sdk import PlatformWalletSdk
from .platform_wallet import PlatformWalletClient
from .session_storage import SessionStorage


async def _platform_signer_address(config: PredictConfig) -> str:
    client = PlatformWalletClient(config)
    try:
        wallet = await asyncio.to_thread(client.get_wallet_address)
...
def make_wallet_sdk(config: PredictConfig) -> WalletSdkProtocol:
    ...
    return PlatformWalletSdk(config)
...
class MandatedWalletStatusSnapshot:
    ...
    sdk_available_tools: list[str]
    sdk_missing_required_tools: list[str]
    sdk_runtime_ready: bool
    vault_address: str
    vault_address_source: str
    vault_deployed: bool
    vault_health: VaultHealthCheckResult | None
    ...
    def to_dict(self) -> dict[str, object]:
        ...
        payload = {
            ...
            "sdk": {
                "availableTools": self.sdk_available_tools,
                "missingRequiredTools": self.sdk_missing_required_tools,
                "runtimeReady": self.sdk_runtime_ready,
            },
            "vaultAddress": self.vault_address,
            "vaultAddressSource": self.vault_address_source,
            "vaultDeployed": self.vault_deployed,
    ...
    *,
    ...
    return {
        ...
        "manualTopUpGuidance": (
            ...
            "is the deposit/recipient identity and receives vault-driven top-ups afterward. "
            "Predict Account buy/withdraw execution is currently unavailable."
        ),
...
def has_mandated_vault_derivation(config: PredictConfig) -> bool:
    return bool(
        ...
        and config.mandated_vault_asset_address
        and config.mandated_vault_name
        and config.mandated_vault_symbol
        and config.mandated_vault_salt
    )
    ...
    payload.update(
        {
            ...
            "ERC_MANDATED_VAULT_AUTHORITY": str(resolved_authority),
...
def _build_bootstrap_snapshot(
    ...
    *,
    ...
    resolved_authority: str | None = None,
    ...
    return MandatedVaultBootstrapSnapshot(
        ...
        backfill_env=_build_backfill_env(
            config, bootstrap, resolved_authority=resolved_authority
        )
...
async def resolve_mandated_vault(
    ...
    *,
    ...
    if config.mandated_vault_address:
        ...
        signer_address = await _platform_signer_address(config)
        authority = config.mandated_vault_authority or signer_address
    ...
    try:
        vault_health = await bridge.health_check(vault_address)
        vault_deployed = True
    except MandatedVaultBridgeError as error:
    ...
    if (
        ...
        prepare_authority = config.mandated_vault_authority or await _platform_signer_address(
            config
        )
        create_vault_prepare = await bridge.prepare_create_vault(
            from_address=prepare_authority,
            factory=config.mandated_factory_address,
            asset=str(config.mandated_vault_asset_address),
            name=str(config.mandated_vault_name),
            symbol=str(config.mandated_vault_symbol),
...
def _is_expected_undeployed_health_error(error: MandatedVaultBridgeError) -> bool:
...
class WalletManager:
    ...
    def get_status(self) -> WalletStatusSnapshot | MandatedWalletStatusSnapshot:
        ...
        return WalletStatusSnapshot(
            ...
            auth_ready=bool(
                self._config.auth_signer_address
                or self._config.platform_signer_configured
            ),
            ...
            try:
                if not _supports_vault_bootstrap(bridge):
                    raise MandatedVaultBridgeError(
                        "Mandated SDK bridge cannot perform bootstrap execute; missing required tool: vault_bootstrap."
                    )
                signer_address = await _platform_signer_address(bootstrap_config)
                authority = (
                    bootstrap_config.mandated_vault_authority or signer_address
                )
                bootstrap = await bridge.vault_bootstrap(
    ...
    def _require_sdk(self) -> WalletSdkProtocol:
        if (
            self._config.auth_signer_address is None
            and not self._config.platform_signer_configured
        ):
            raise ConfigError(
                "Wallet actions require the platform wallet API (WALLET_API_URL, WALLET_API_TOKEN, INSTANCE_ID)."
            )
        ...
        async def _load_and_close() -> MandatedWalletStatusSnapshot:
            ...
            try:
                ...
                return MandatedWalletStatusSnapshot(
                    ...
                    sdk_available_tools=sorted(bridge.available_tools),
                    sdk_missing_required_tools=sorted(bridge.missing_required_tools),
                    sdk_runtime_ready=bridge.runtime_ready,
                    vault_address=resolution.vault_address,
                    vault_address_source=resolution.vault_address_source,
