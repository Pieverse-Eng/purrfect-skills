...
- Settlement is not always instant; do not claim Arbitrum credit until balance
  checks or the user accepts pending settlement.
...
4. Later, if the user asks to verify:
...
- **Never re-run deposit or withdraw** after network timeout, unknown bridge
  status, or `broadcast_unknown`. Reconcile with balances and request ids
  instead. A second call can double-spend.
- Do not describe `paying` / pending bridge states as completed.
