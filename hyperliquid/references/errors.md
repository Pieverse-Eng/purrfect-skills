...
| `HYPERLIQUID_TRADING_DISABLE_BLOCKED` | Platform refused disable because positions, orders, or balances block it | Present `blockers`; clear them; re-confirm disable. Do not recompute the rule or retry the same blockers in a loop |
...
| Missing `--always-place true` for trigger, `Ioc`, or `FrontendMarket` modification | Verify the exact open target and obtain confirmation of the always-place duplicate-order risk before retrying |
| `--always-place false` | Use `true` only when required; otherwise omit the option |
