...
1. After an `*-execute` that may have started, resume with the **same**
   `--preview-id` and the same execute command. Do not create a new preview.
   Resume when JSON `status` is `in_progress`, `broadcasted`,
   `chain_confirmed`, or `upstream_pending`. `replayed: true` means this
   already ran — report it; do not treat it as a new execute.
...
| `PREDICT_PREVIEW_NOT_FOUND` | Preview id does not exist | Re-preview if you never executed; if you did, reconcile with `orders` / `positions` — do not guess another id |
| `PREDICT_PREVIEW_STALE` | Market, fees, tokens, or MARKET book moved | New preview from live book; new confirmation |
| `PREDICT_PREVIEW_ACTION_MISMATCH` / `PREDICT_PREVIEW_SCOPE_MISMATCH` | Execute route or cancel/approval mode does not match the preview | Use the matching `*-execute` for that preview; do not swap cancel ↔ remove-from-book |
| `PREDICT_ORDER_BELOW_MINIMUM` | Qty &lt; 0.01, value &lt; 0.9 USDT, or MARKET BUY spend &lt; 1 | Ask for a valid size; do not pad silently |
...
| `PREDICT_APPROVAL_ALREADY_SET` | Requested allowance/operator already matches | Stop; treat as approved; do not execute |
...
| `PREDICT_MARKET_NOT_OPEN` / `PREDICT_INVALID_MARKET` | Market closed or payload unusable | Report; do not force an order |
...
| `PREDICT_REMOVE_FROM_BOOK_REJECTED` | One or more hashes not removed | Report per-hash results; do not call it a full success |
| `PREDICT_REFERRAL_ALREADY_SET` / `PREDICT_REFERRAL_LOCKED` | Different code set, or settings locked | Report; do not retry another code unless the user asks |
...
| `POLICY_DEFERRED` | Wallet policy parked the write | After approval, same `*-execute --preview-id` (or same `--code` for referral). Do not preview again |
...
| `PREDICT_*_TRANSACTION_REVERTED` | On-chain revert | Show returned txs; do not resubmit blindly |
...
| `PREDICT_AUTH_REJECTED` | Predict rejected the platform JWT | Report; do not ask the user for a Predict API key |
...
| Unsupported / duplicate / positional argument | CLI rejected before HTTP | Fix flags; do not invent aliases |
